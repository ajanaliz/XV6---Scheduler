...
int main(void) {
  ...
  19:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    ...
    for (int i = 0; i < 10; i++) {
        childPid[i] = fork();
  20:	e8 d5 02 00 00       	call   2fa <fork>
        if (childPid[i] < 0) {
  25:	85 c0                	test   %eax,%eax
  27:	78 5b                	js     84 <main+0x84>
            printf(1, "fork failed\n");
            exit();
        } else if (childPid[i] == 0) {
  29:	74 6c                	je     97 <main+0x97>
    int childPid[10];
    for (int i = 0; i < 10; i++) {
        childPid[i] = 0;
    }
  ...
  43:	57                   	push   %edi
  44:	56                   	push   %esi
  45:	e8 58 03 00 00       	call   3a2 <getppid>

        printf(1, "child %d : ", i);
  4a:	83 c4 0c             	add    $0xc,%esp
  4d:	53                   	push   %ebx
  4e:	68 8d 07 00 00       	push   $0x78d
        } else if (childPid[i] > 0) {
            continue;
        }
    }
  ...
  53:	83 c3 01             	add    $0x1,%ebx
        ...
        printf(1, "child %d : ", i);
  56:	6a 01                	push   $0x1
  58:	e8 03 04 00 00       	call   460 <printf>
        printf(1, "wtime : %d , rtime : %d , turnaround time : %d\n\n", wtime, rtime, wtime + rtime);
  5d:	8b 45 e0             	mov    -0x20(%ebp),%eax
  60:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  63:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
  66:	89 0c 24             	mov    %ecx,(%esp)
  69:	52                   	push   %edx
  6a:	50                   	push   %eax
  6b:	68 c0 07 00 00       	push   $0x7c0
  70:	6a 01                	push   $0x1
  ...
  7d:	75 c1                	jne    40 <main+0x40>
            ...
            exit();
  7f:	e8 7e 02 00 00       	call   302 <exit>
    }
  ...
  92:	e8 6b 02 00 00       	call   302 <exit>
            ...
            int pid = getpid();
  97:	e8 e6 02 00 00       	call   382 <getpid>
            for (int j = 0; j < 10; j++) {
  ...
  a2:	83 c3 01             	add    $0x1,%ebx
                printf(1, "Child %d prints for the %d time\n", pid, j);
  a5:	68 9c 07 00 00       	push   $0x79c
  aa:	6a 01                	push   $0x1
  ac:	e8 af 03 00 00       	call   460 <printf>
        if (childPid[i] < 0) {
            printf(1, "fork failed\n");
            exit();
        } else if (childPid[i] == 0) {
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3ff:	f7 f6                	div    %esi
 401:	0f b6 92 fc 07 00 00 	movzbl 0x7fc(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 578:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 57b:	b8 f4 07 00 00       	mov    $0x7f4,%eax
...
free(void *ap)
{
 ...
 67a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 67d:	a3 a0 0a 00 00       	mov    %eax,0xaa0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 73c:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 73f:	89 15 a0 0a 00 00    	mov    %edx,0xaa0
      return (void*)(p + 1);
 ...
 754:	eb e9                	jmp    73f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 756:	c7 05 a0 0a 00 00 a4 	movl   $0xaa4,0xaa0
 75d:	0a 00 00 
 760:	c7 05 a4 0a 00 00 a4 	movl   $0xaa4,0xaa4
 767:	0a 00 00 
    base.s.size = 0;
 76a:	b8 a4 0a 00 00       	mov    $0xaa4,%eax
 76f:	c7 05 a8 0a 00 00 00 	movl   $0x0,0xaa8
 776:	00 00 00 
